...
# pattern = (not start with number/.) (mm) / (dd) [/ (YYYY | YY) optional] (not end with another number/%)
...
date_pattern_reg = re.compile(date_pattern)
...
def check_for_date(patient,note,chunk, output_handle):
    ...
    for match in date_pattern_reg.finditer(chunk):
