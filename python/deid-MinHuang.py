import re
import sys
#date_pattern ='(\d{3}[-\.\s/]??\d{3}[-\.\s/]??\d{4}|\(\d{3}\)\s*\d{3}[-\.\s/]??\d{4})'
date_pattern ='(\d\d?[-\./]\d\d?[-\./]\d{2}\d?\d?|\d[\./]\d\d?｜[1][0-2][\./]\d\d?|[1-2]\d|[3][0-1])'
        ...
        with open(text_path) as text:
            ...
            for line in text:
                ...
                if len(record_end):
                    ...
                    # pass all to check_for_date to find any date numbers in note.
